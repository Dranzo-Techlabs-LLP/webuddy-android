...
internal fun MessageComposerView(
    ...
    if (state.showNewSessionWarning) {
        NewSessionWarningDialog(
            onConfirm = { neverRemindAgain ->
                state.eventSink(MessageComposerEvent.ConfirmNewSession(neverRemindAgain))
            },
            onDismiss = {
                state.eventSink(MessageComposerEvent.DismissNewSessionWarning)
            },
        )
    }
}
...
@Composable
private fun NewSessionWarningDialog(
    onConfirm: (neverRemindAgain: Boolean) -> Unit,
    onDismiss: () -> Unit,
) {
    var neverRemindAgain by remember { mutableStateOf(false) }
    AlertDialog(
        ...
        text = {
            Column {
                Text(
                    text = "Your previous session has ended. Sending this message starts a new " +
                        "billable session and places a fresh hold on your credits.",
                ...
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { neverRemindAgain = !neverRemindAgain }
                        .padding(top = 12.dp),
                    verticalAlignment = Alignment.CenterVertically,
                ) {
                    Checkbox(
                        checked = neverRemindAgain,
                        onCheckedChange = { neverRemindAgain = it },
                    )
                    Spacer(Modifier.width(4.dp))
                    Text(
                        text = "Never remind me again for this consultant",
                        style = ElementTheme.typography.fontBodyMdRegular,
        ...
        confirmButton = {
            TextButton(onClick = { onConfirm(neverRemindAgain) }) {
                Text("Continue")
            }
        },
        dismissButton = {
